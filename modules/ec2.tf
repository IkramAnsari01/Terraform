...
resource "aws_key_pair" "aws-key"{
	key_name = "modular-${var.env}-Terraform-Key"
	public_key = file("newKey.pub")

	tags = {
		Name = "modular-${var.env}-Terraform-Key"
		Environment = var.env
	}
...
resource aws_security_group my-security-grp{
	name = "modular-${var.env}-Terra_security_grp"
...
resource "aws_instance" "my_instance"{
	...
	instance_type = var.aws_instance_type   #"t3.micro"
	ami = var.aws_ami_id  #"ami-0974a2c5ddf10f442"

	user_data = file("install_ngnix.sh")

	root_block_device{
		volume_size = var.aws_root_storage_size  #each.value == "t3.micro" ? 15 : 20 ##var.aws_root_storage_size  #15
		volume_type = "gp3"
	}
	
	tags = {
		Name = "modular-${var.env}-Terra-Created-Instance"
		Environment = var.env
	}
